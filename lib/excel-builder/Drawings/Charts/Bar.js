function Bar() {
    ...
    this._valAx = {
        ...
        numFmt : {
            sourceLinked : '0',
            formatCode : '[>=1000000]#.0,,"M";[>=1000]#.0,"k";General'
        },
        min : '0',
        max : null
    };
...
_.extend(Bar.prototype, {
    ...
    setChartProperty: function(grouping, gapWidth, overlap){
        if(grouping){
            this.chartProp.grouping = grouping;
        }

        if(gapWidth){
            this.chartProp.gapWidth = gapWidth;
        }

        if(overlap){
            this.chartProp.overlap = overlap;
        }
    },
    ...
    setCatAx_titleVisible: function(val){
        this._catAx.titleVisible = val;
    },
    ...
    setTitle: function(xmlDoc, txRef, fontSize){
        ...
        if(txRef){
            ...
            txF.appendChild(xmlDoc.createTextNode(txRef));
        ...
        title.appendChild(this.getTextProperties(xmlDoc, fontSize));
    ...
    setCatAx: function(xmlDoc){
        ...
        if(this._catAx.titleVisible){
            var txRef = '\''+ this.sheetName + '\'' + '!D1';
            catAx.appendChild(this.setTitle(xmlDoc, txRef, '1000'));
        }
    ...
    setChart: function(xmlDoc){
        var Chart = util.createElement(xmlDoc, 'c:barChart');

        Chart.appendChild(util.createElement(xmlDoc, 'c:barDir', [
            ['val', 'col']
        ]));
        Chart.appendChild(util.createElement(xmlDoc, 'c:grouping', [
            ['val', this.chartProp.grouping]
        ]));
        ...
        Chart.appendChild(util.createElement(xmlDoc, 'c:gapWidth', [
            ['val', this.chartProp.gapWidth]
        ]));
        Chart.appendChild(util.createElement(xmlDoc, 'c:overlap', [
            ['val', this.chartProp.overlap]
        ]));
    ...
    setLegend: function(xmlDoc){
        ...
        legend.appendChild(this.getTextProperties(xmlDoc, '900'));
    ...
    addSeries: function(xmlDoc, txFormula, catFormula, valFormula, seq){
        ...
        series.appendChild(util.createElement(xmlDoc, 'c:invertIfNegative', [
            ['val', '0']
        ]));
    ...
    toXML: function () {
        ...
        if(this.titleVisible){
            var title = this.setTitle(xmlDoc, null, '1100');
