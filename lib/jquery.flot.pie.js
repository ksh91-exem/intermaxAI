...
//						ctx.fillStyle = getColorOrGradient(options.series.pie.gradient, plot.height(), 0, color);
					} else {
				...
				function drawLabels() {
					...
					function drawLabel(slice, startAngle, index) {
						...
						// 라벨이 캔버스의 반 넓이보다 클경우 캔버스 반 넓이로 설정
						if(label.width() > canvasWidth / 2){
							label.width(canvasWidth / 2);
						}
						
						var labelTop = (y - label.height() / 2);
						var labelLeft = (x - label.width() / 2);
						// 라벨 넓이가 길어 캔버스 왼쪽 포지션을 벗어나는 경우를 방지하기 위해
						if(labelLeft < 0){
							labelLeft = 0;
						}
						...
						// 라벨 넓이가 길어 현재 포지션에서 캔버스 오늘쪽 영역을 벗어나는 경우
						if(canvasWidth - labelLeft < label.width()){
							label.css('max-width', canvasWidth - labelLeft - 30);
						}
