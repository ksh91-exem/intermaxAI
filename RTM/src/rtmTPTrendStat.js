Ext.define('rtm.src.rtmTPTrendStat', {
    extend: 'Exem.DockForm',
    title : 'TP ' + common.Util.TR('Performance Stat'),
    layout: 'fit',
    ...
    initProperty: function() {
        ...
        this.wasStatNameList = Comm.RTComm.getTPStatIdArr();
    ...
    init: function() {
        ...
        for (ix = 0; ix < chartCount; ix++) {
            this.chartList[ix] = Ext.create('rtm.src.rtmTPTrendStatChart');
            ...
            this.chartList[ix].setTitle(Comm.RTComm.getTPStatNameById(this.psWasStatList[ix]));
    ...
    initViewStat: function() {
        ...
        if (Comm.web_env_info.Intermax_RTM_TP_WasStatList) {
            Comm.RTComm.saveWasStatList(this.componentId, Comm.web_env_info.Intermax_RTM_TP_WasStatList);
            common.WebEnv.del_config('Intermax_RTM_TP_WasStatList');
        }
        ...
        for (ix = 0, ixLen = realtime.TPStatName.length; ix < ixLen; ix++) {
            if (this.psWasStatList.indexOf(realtime.TPStatName[ix].id) === -1) {
                this.psWasStatList[index] = realtime.TPStatName[ix].id;
            }
        }
    },
    ...
    initTrendData: function() {
        ...
        for (ix = 0, ixLen = this.serverIdArr.length; ix < ixLen; ix++) {
            ...
            if (!Repository.tmadminChartData[wasid]) {
                Repository.tmadminChartData[wasid] = {};
                Repository.tmadminDataLog[wasid]     = {};

                for (jx = 0, jxLen = this.wasStatNameList.length; jx < jxLen; jx++) {
                    statName = this.wasStatNameList[jx];

                    Repository.tmadminChartData[wasid][statName] = 0;
                    Repository.tmadminDataLog[wasid][statName]     = [];

                    for (kx = 0, kxLen = this.psDataCount; kx < kxLen; kx++) {
                        Repository.tmadminDataLog[wasid][statName].push(null);
                    }
                }
            }
        }
        ...
        for (ix = 0, ixLen = this.psWasStatList.length; ix < ixLen; ix++) {
            ...
            for (jx = 0, jxLen = this.serverIdArr.length; jx < jxLen; jx++) {
                ...
                for (cx = 0, cxLen = this.psDataCount; cx < cxLen; cx++) {
                    statValue = isDown? null : Repository.tmadminDataLog[serverId][statname][cx];

                    try {
                        if (!Repository.tmadminDataLog[serverId][statname]) {
    ...
    drawChart: function() {
        ...
        for (var ix = 0, ixLen = this.chartList.length; ix < ixLen; ix++) {
            ...
            statName = Comm.RTComm.getTPStatNameById(statId);

            if (this.chartList[ix].title !== statName) {
                this.chartList[ix].setTitle(statName);
            }
            this.chartList[ix].drawData(Repository.tmadminChartData.timeRecordData, this.psStatData.statdata[ix], this.chartFixed, this.chartMaxValue);
        }
    ...
    changeOption: function() {
        ...
        if (chartCount - this.psWasStatList.length > 0) {
            for (ix = this.psWasStatList.length; ix < chartCount; ix++) {
                ...
                for (kx = 0; kx < realtime.defaultTPStatName.length; kx++) {
                    if (this.psWasStatList.indexOf(realtime.defaultTPStatName[kx].id) === -1) {
                        this.psWasStatList[ix] = realtime.defaultTPStatName[kx].id;
                        break;
                    }
                }
            }
    ...
    loadStatData: function(index, title) {
        ...
        this.tmpChart.setTitle(Comm.RTComm.getTPStatNameById(title));
    ...
    openTPServerList: function() {
        ...
        var time = Repository.tmadminChartData.timeRecordData[timeIndex];
        ...
        var tpServerList = Ext.create('rtm.src.rtmTPServerList');
        tpServerList.retrieveTime = time;
        tpServerList.statName     = statName;

        tpServerList.initWindow();
