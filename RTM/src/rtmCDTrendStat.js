Ext.define('rtm.src.rtmCDTrendStat', {
    extend: 'Exem.DockForm',
    title : common.Util.TR('Performance Stat'),
    layout: 'fit',

    chartRows : 2,      // 가로 갯수  Max 2
    chartCols : 2,      // 세로 갯수  Max 2
    ...
    initProperty: function() {
        ...
        this.wasStatNameList = Comm.RTComm.getCDStatIdArr();
    ...
    init: function() {
        ...
        for (ix = 0; ix < chartCount; ix++) {
            this.chartList[ix] = Ext.create('rtm.src.rtmCDTrendStatChart');
            ...
            this.chartList[ix].setTitle(Comm.RTComm.getCDStatNameById(this.psWasStatList[ix]));
    ...
    initViewStat: function() {
        ...
        if (Comm.web_env_info.Intermax_RTM_CD_WasStatList) {
            Comm.RTComm.saveWasStatList(this.componentId, Comm.web_env_info.Intermax_RTM_CD_WasStatList);
            common.WebEnv.del_config('Intermax_RTM_CD_WasStatList');
        }
            ...
            if (!Comm.web_env_info[envKey]) {
                ...
                if (this.psWasStatList.length < statCount) {
                    for (ix = this.psWasStatList.length; ix < statCount; ix++) {
                        this.psWasStatList[this.psWasStatList.length] = realtime.defaultCDStatName[ix].id;
                    }
                    Comm.RTComm.saveWasStatList(this.componentId, this.psWasStatList.join(','));
                }
            }
        ...
        console.log(realtime.TPStatName, this.psWasStatList);

        for (ix = 0, ixLen = realtime.CDStatName.length; ix < ixLen; ix++) {
            if (this.psWasStatList.indexOf(realtime.CDStatName[ix].id) === -1) {
                this.psWasStatList[index] = realtime.CDStatName[ix].id;
            }
        }
    },
        ...
        if (Comm.web_env_info[envKey]) {
            ...
            if (envWasStatList.length < statCount) {
                for (var ix = envWasStatList.length; ix < statCount; ix++) {
                    envWasStatList[envWasStatList.length] = realtime.defaultCDStatName[ix].id;
                }
            }
        }
        ...
        for (ix = 0, ixLen = this.serverIdArr.length; ix < ixLen; ix++) {
            ...
            if (!Repository.CDTrendData[serverId]) {
                ...
                for (jx = 0, jxLen = this.wasStatNameList.length; jx < jxLen; jx++) {
                    ...
                    Repository.CDTrendData[serverId][statName] = 0;
                    Repository.CDTrendDataLog[serverId][statName]   = [];

                    for (kx = 0, kxLen = this.psDataCount; kx < kxLen; kx++) {
                        Repository.CDTrendDataLog[serverId][statName].push(null);
                    }
                }
            }
        }
    ...
    drawData: function() {
        var ix, jx, kx, cx, ixLen;
        var serverId, statname, statValue;
        var isDown;
        ...
        for (ix = 0, ixLen = this.psWasStatList.length; ix < ixLen; ix++) {
            ...
            for (jx = 0; jx < serverCount; jx++) {
                ...
                if (!this.psStatData.statdata[ix][jx] && this.psStatData.statdata[ix].length < this.serverIdArr.length) {
                    this.psStatData.statdata[ix][jx]    = [];
                    this.psStatData.statdata[ix][jx][0] = serverId;   // Server ID
                ...
                isDown = Comm.RTComm.isDownByServer(serverId, 'CD');

                for (cx = 0; cx < this.psDataCount; cx++) {
                    statValue = isDown? null : Repository.CDTrendDataLog[serverId][statname][cx];
    ...
    drawChart: function() {
        ...
        for (var ix = 0, ixLen = this.chartList.length; ix < ixLen; ix++) {
            ...
            statName = Comm.RTComm.getCDStatNameById(statId);

            if (this.chartList[ix].title !== statName) {
                this.chartList[ix].setTitle(statName);
            }
            this.chartList[ix].drawData(Repository.CDTrendData.timeRecordData, this.psStatData.statdata[ix], this.chartFixed, this.chartMaxValue);
        }
    },
    ...
    changeOption: function() {
        ...
        if (chartCount - this.psWasStatList.length > 0) {
            for (ix = this.psWasStatList.length; ix < chartCount; ix++) {
                ...
                for (kx = 0; kx < realtime.defaultCDStatName.length; kx++) {
                    if (this.psWasStatList.indexOf(realtime.defaultCDStatName[kx].id) === -1) {
                        this.psWasStatList[ix] = realtime.defaultCDStatName[kx].id;
                        break;
                    }
                }
            }
    ...
    loadStatData: function(index, title) {
        ...
        this.tmpChart.setTitle(Comm.RTComm.getCDStatNameById(title));
