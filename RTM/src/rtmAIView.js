Ext.define('rtm.view.rtmAIView', {
    ...
    iniProperty: function() {
        this.defaultView = '../images/View_Image/rtmAIViewDefault.png';
    ...
    createView: function() {
        ...
        Comm.RTComm.checkChangeDefaultAILayout();

        var defaultLayer = Comm.RTComm.getDockLayer('AIMonitor');
    ...
    resetPAWebInfo: function() {
        ...
        var default_gc, default_stat,
            ...
            default_db_stat, default_db_wait, default_db_ratio, default_db, default_wait,
            default_ai_stat, default_ai_stat_id,
            keys, key, ix, ixLen;

        if ( !Comm.web_env_info['pa_performance_trend_stat_Default']) {

            //DB Trend variable
            default_db_stat  = JSON.stringify([
                'session logical reads'
                , 'physical reads'
                , 'execute count'
                , 'redo entries'
            ]);
            default_db_wait  = JSON.stringify([
                'Latch Wait Time (Total)'
                , 'db file sequential read'
                , 'db file scattered read'
                , 'library cache pin'
            ]);
            default_db_ratio = JSON.stringify([
        ...
        if (!Comm.web_env_info['pa_was_performance_trend_all_load_db_Default']) {
            default_ai_stat = {
                load_was : ['Active Transaction Count', 'JVM CPU Usage (%)', 'JVM Used Heap (MB)', 'Transaction Count', 'Transaction Elapse Time (AVG)'],
                ano_was  : ['Active Transaction Count', 'JVM CPU Usage (%)', 'JVM Used Heap (MB)', 'Transaction Count', 'Transaction Elapse Time (AVG)'],
                load_db  : ['CPU Usage', 'Session Logical Reads', 'DB Time', 'Non Idle Wait Time', 'Physical Reads'],
                ano_db   : ['CPU Usage', 'DB Time', 'Non Idle Wait Time', 'Physical Reads', 'Physical Writes'],
                load_txn : ['Transaction Execution Count', 'Transaction Elapse Time', 'Transaction CPU TIME'],
                load_biz : ['Transaction Execution Count', 'Transaction Elapse Time', 'Transaction CPU TIME'],
                ano_txn  : ['Transaction Execution Count', 'Transaction Elapse Time', 'Transaction CPU TIME']
            };

            default_ai_stat_id = {
                load_was : ['active_txns', 'jvm_cpu_usage', 'jvm_used_heap', 'txn_end_count', 'txn_elapse'],
                ano_was  : ['active_txns', 'jvm_cpu_usage', 'jvm_used_heap', 'txn_end_count', 'txn_elapse'],
                load_db  : ['os_cpu', 'logical_read', 'db_time', 'nonidle_wait_time', 'physical_read'],
                ano_db   : ['user_cpu', 'db_time', 'non_idle_wait_time', 'physical_reads', 'physical_writes'],
                load_txn : ['txn_exec_count', 'txn_elapse', 'txn_cpu_time'],
                load_biz : ['txn_exec_count', 'txn_elapse', 'txn_cpu_time'],
                ano_txn  : ['txn_exec_count', 'txn_elapse', 'txn_cpu_time']
            };

            keys = Object.keys(default_ai_stat);
            for (ix = 0, ixLen = keys.length; ix < ixLen; ix++) {
                key = keys[ix];
                default_stat = JSON.stringify(default_ai_stat[key]);
                default_stat_id = JSON.stringify(default_ai_stat_id[key]);

                common.WebEnv.insert_config('pa_was_performance_trend_all_' + key + '_types'    , 'Default');
                common.WebEnv.insert_config('pa_was_performance_trend_all_' + key + '_last_type', 'Default');

                common.WebEnv.insert_config('pa_was_performance_trend_all_' + key        , default_stat   , 'Default');
                common.WebEnv.insert_config('pa_was_performance_trend_all_' + key + '_id', default_stat_id, 'Default');
            }
