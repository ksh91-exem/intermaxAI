Ext.define('rtm.src.rtmBizTPSMonitor', {
    extend: 'Exem.DockForm',
    title : common.Util.TR('TPS'),
    layout: 'fit',
    id    : 'rtmBizTPSMonitor',
    ...
    init: function() {
        ...
        for (ix = 0; ix < this.tierIdArr.length; ix++) {
            this.chartList[ix] = Ext.create('rtm.src.rtmBizTPSMonitorChart');
            this.chartList[ix].target = Ext.getCmp(this.chartId[ix]);
            this.chartList[ix].parent = this;
            this.chartList[ix].statName = 'TPS';

            this.chartList[ix].setTitle('TPS');
        ...
        for (ix = 0, ixLen = bizIdList.length; ix < ixLen; ix++) {
            ...
            for (jx = 0, jxLen = tierIdList.length; jx < jxLen; jx++) {
                ...
                for (kx = 0, kxLen = this.dataCount; kx < kxLen; kx++) {
                    statValue = Repository.BizTrendDataLog[bizId][tierId].TPS[kx];
