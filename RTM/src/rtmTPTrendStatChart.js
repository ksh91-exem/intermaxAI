Ext.define('rtm.src.rtmTPTrendStatChart', {
    ...
    settDblClickEvent: function() {
        this.chart.chartContainer.addEventListener('dblclick', function(e) {
            ...
            this.parent.openTPServerList(this.statName, index);
    ...
    setChartSeries: function() {
        ...
        if (this.isTotal === true) {
            ...
            for (var ix = 0, ixLen = Comm.tpNameArr.length ; ix < ixLen; ix++) {

                if (this.chartOption) {
                    lineWidth = this.chartOption[Comm.tpNameArr[ix]].lineWidth;
                }

                labelColor = realtime.serverColorMap[this.parent.openViewType][Comm.tpIdArr[ix]];

                this.chart.addSeries({
                    id       : Comm.tpIdArr[ix],
                    label    : Comm.tpNameArr[ix],
                    color    : labelColor,
                    lineWidth: lineWidth
                });
            }
    ...
    setVisibleSeries: function(visibleSeries) {
        ...
        for (var ix = 0, ixLen = Comm.tpNameArr.length ; ix < ixLen; ix++) {
            if (isAllVisible === true) {
                ...
                isVisible = Ext.Array.contains(visibleSeries, Comm.tpNameArr[ix]);
            }
    ...
    openStatList: function(statname) {
        this.listWindow = Ext.create('rtm.src.rtmTPStatList', {
            style: {'z-index': '10'}
        });

        this.listWindow.statName     = statname;
        this.listWindow.oldStatName  = Comm.RTComm.getTPStatIdByName(statname);
