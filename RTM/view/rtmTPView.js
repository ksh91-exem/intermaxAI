Ext.define('rtm.view.rtmTPView', {
    ...
    listeners: {
        ...
        show: function() {
            ...
            window.isLockRTMFrame = Comm.RTComm.getBooleanValue(Comm.web_env_info.rtm_tp_frame_lock);
    ...
    iniProperty: function() {
        this.defaultView = '../images/View_Image/rtmTPViewDefault.png';
    ...
    setRealtimeViewLock: function() {
        if (!Comm.web_env_info.rtm_tp_frame_lock) {
            common.WebEnv.Save('rtm_tp_frame_lock', false);
            window.isLockRTMFrame = false;

        } else {
            window.isLockRTMFrame = (Comm.web_env_info.rtm_tp_frame_lock.toLowerCase() === 'true');
    ...
    createView: function() {
        ...
        if (Ext.isEmpty(realtime.TPHostList)) {
            this.selectedType = 1;
        }
        ...
        if (Comm.RTComm.checkChangeDefaultTPLayout) {
            Comm.RTComm.checkChangeDefaultTPLayout();
        }

        var defaultLayer = Comm.RTComm.getDockLayer('TPMonitor');
    ...
    createLeftLayer: function() {
        ...
        var leftCollapse;
        if (Comm.web_env_info.rtm_tp_leftMenuCollapse && Comm.web_env_info.rtm_tp_leftMenuCollapse === 'true') {
            leftCollapse = true;
        } else {
            leftCollapse = false;
        }
        ...
        this.leftCon = Ext.create('Exem.Panel',{
            ...
            listeners: {
                ...
                collapse: function(p) {
                    ...
                    common.WebEnv.Save('rtm_tp_leftMenuCollapse', true);
                ...
                beforeexpand: function() {
                    ...
                    common.WebEnv.Save('rtm_tp_leftMenuCollapse', false);
                },
                expand: function() {
                    if (this.leftCon.isExpandLock === true) {
                        return false;
    ...
    setServerColors: function() {
        var ix, ixLen;
        var viewIndex   = Comm.web_env_info['xm-dock-save-rtm.view.rtmTPView'];
        ...
        if (serverColor) {
            ...
            if (customColor && !Array.isArray(customColor)) {
                realtime.serverColorMap.TP = customColor.TP;
            }
            customColor = null;
            serverColor = null;
        }
        ...
        if (!realtime.serverColorMap.TP) {
            realtime.serverColorMap.TP = {};
        }

        for(ix = 0, ixLen = Comm.tpIdArr.length; ix < ixLen; ix++ ) {
            serverId   = Comm.tpIdArr[ix];
            color      = realtime.serverColorMap.TP[serverId];
            ...
            realtime.serverColorMap.TP[serverId] = color;

            if (!Comm.tpInfoObj[Comm.tpIdArr[ix]].labelColor){
                Comm.tpInfoObj[Comm.tpIdArr[ix]].labelColor = realtime.Colors[ix];
            }
        }
    },
    ...
    setMenuGroupList: function() {
        ...
        var groupName;
        var serverCount = Comm.tpIdArr.length;
        var serverID;
        ...
        for (jx = 0; jx < serverCount; jx++) {
            ...
            serverList[serverList.length] = {
                ...
                serverType   : 'TP',
                labelColor   : realtime.serverColorMap.TP[serverID]
            };
        }

        // Host List
        for (ix = 0, ixLen = realtime.TPHostList.length; ix < ixLen; ix++) {
            groupName = realtime.TPHostList[ix];
        ...
        // Business List
        for (ix = 0, ixLen = Comm.tpBizGroups.length; ix < ixLen; ix++) {
            groupName = Comm.tpBizGroups[ix];
        ...
        this.menuBarGroup.frameChange = function() {
            rtmBase.frameChange(realtime.selectedTPNames);
        };
        serverList = null;
    ...
    wasSelect: function(serverName) {
        ...
        if (Ext.isEmpty(serverName)) {
            Comm.selectedTpArr.length = 0;
        }

        var serverId  = Comm.RTComm.getTPIdByName(serverName);
        var isContain = Ext.Array.contains(Comm.selectedTpArr, serverId);

        if (Comm.selectedTpArr.length === Comm.tpIdArr.length) {
            Comm.selectedTpArr.length = 0;
            realtime.selectedTPNames.length = 0;

            Comm.selectedTpArr.push(serverId);
            realtime.selectedTPNames.push(serverName);

        } else if (isContain || Ext.isEmpty(serverName)) {
            Ext.Array.remove(Comm.selectedTpArr, serverId);
            Ext.Array.remove(realtime.selectedTPNames, serverName);

            if (Comm.selectedTpArr.length === 0) {
                Comm.selectedTpArr = Comm.tpIdArr.concat();
            }
        ...
        this.frameChange(realtime.selectedTPNames);
    },
    ...
    frameRefresh: function() {
        ...
        if (Comm.rtmTPShow === true) {
            var serverId;
            var data;

            for (var ix = 0, ixLen = Comm.tpIdArr.length; ix < ixLen; ix++) {
                serverId = Comm.tpIdArr[ix];
    ...
    startFrameDraw: function() {
        ...
        if (!this.dockLayer || !Comm.rtmTPShow) {
            return;
        }
        ...
        if (windowFloatingList.length) {
            for (ix = 0, ixLen = windowFloatingList.length; ix < ixLen; ix++) {
                ...
                if (windowComponent && windowComponent.frameRefresh && windowComponent.openViewType === 'TP') {

                    if (windowComponent.title === common.Util.TR('Activity Monitor') && Comm.rtmTPShow) {
        ...
        // TP 모니터링 화면에 표시되고 있는 각 컴포넌트를 새로고침한다.
        for(ix = 0, ixLen = this.dockLayer.dockList.length; ix < ixLen; ix++){
    ...
    resetPAWebInfo: function() {
            ...
            for (var ix = 0, ixLen = keys.length; ix < ixLen; ix++) {
                ...
                if (!Ext.isObject(envData) && !Ext.isArray(envData)) {
                    try {
                        ...
                        console.debug('%c [RTM TPView] Is String format data.', 'color:blue;', envData);
                    }
                }
            }
        };
        ...
        if ( !Comm.web_env_info['pa_tp_performance_trend_stat_Default']) {
            ...
            common.WebEnv.insert_config( 'pa_tp_performance_trend_types',     'Default' ) ;
            common.WebEnv.insert_config( 'pa_tp_performance_trend_last_type', 'Default' ) ;

            common.WebEnv.insert_config( 'pa_tp_performance_trend_stat', default_stat, 'Default' ) ;
            common.WebEnv.insert_config( 'pa_tp_performance_trend_stat_id', default_stat_id, 'Default' ) ;
            ...
            if (Comm.web_env_info.pa_tp_performance_trend_last_type == null) {
                common.WebEnv.insert_config( 'pa_tp_performance_trend_last_type', 'Default' ) ;
            }
            if (Comm.web_env_info.pa_tp_performance_trend_types == null) {
                common.WebEnv.insert_config( 'pa_tp_performance_trend_types', 'Default' ) ;
            }
        }

        if (!Comm.web_env_info['pa_tp_performance_trend_all_stat_Default']) {
            default_stat = JSON.stringify([
                'TPS'
                ,'Elapse Time'
                ,'Queuing Count'
            ...
            common.WebEnv.insert_config( 'pa_tp_performance_trend_all_types',     'Default' ) ;
            common.WebEnv.insert_config( 'pa_tp_performance_trend_all_last_type', 'Default' ) ;

            common.WebEnv.insert_config( 'pa_tp_performance_trend_all_stat', default_stat, 'Default' ) ;
            common.WebEnv.insert_config( 'pa_tp_performance_trend_all_stat_id', default_stat_id, 'Default' ) ;

            isExecuteReconfig = true;
        }

        if ( !Comm.web_env_info['pa_tp_comparison_stat_Default']) {
            ...
            common.WebEnv.insert_config( 'pa_tp_comparison_types',     'Default' ) ;
            common.WebEnv.insert_config( 'pa_tp_comparison_last_type', 'Default' ) ;

            common.WebEnv.insert_config( 'pa_tp_comparison_stat'   , default_comparison_stat  , 'Default' ) ;
            common.WebEnv.insert_config( 'pa_tp_comparison_stat_id', default_comparison_stat_id, 'Default' ) ;
