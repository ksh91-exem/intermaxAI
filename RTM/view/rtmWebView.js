Ext.define('rtm.view.rtmWebView', {
    ...
    listeners: {
        ...
        show: function() {
            ...
            Comm.rtmWebShow = true;

            window.isLockRTMFrame = Comm.RTComm.getBooleanValue(Comm.web_env_info.rtm_web_frame_lock);
    ...
    setRealtimeViewLock: function() {
        if (!Comm.web_env_info.rtm_web_frame_lock) {
            common.WebEnv.Save('rtm_web_frame_lock', false);
            window.isLockRTMFrame = false;

        } else {
            window.isLockRTMFrame = (Comm.web_env_info.rtm_web_frame_lock.toLowerCase() === 'true');
    ...
    createView: function() {
        ...
        if (Ext.isEmpty(realtime.WebHostList)) {
            this.selectedType = 1;
        }
        ...
        if (Comm.RTComm.checkChangeDefaultWebLayout) {
            Comm.RTComm.checkChangeDefaultWebLayout();
        }

        var defaultLayer = Comm.RTComm.getDockLayer('WebMonitor');
        ...
        window.imxWebDockLayerBaseId = this.dockSite.id;
        window.imxWebBaseViewId      = this.id;
    },
    ...
    createLeftLayer: function() {
        ...
        var leftCollapse;
        if (Comm.web_env_info.rtm_web_leftMenuCollapse && Comm.web_env_info.rtm_web_leftMenuCollapse === 'true') {
            leftCollapse = true;
        } else {
            leftCollapse = false;
        }
        ...
        this.leftCon = Ext.create('Exem.Panel',{
            ...
            listeners: {
                ...
                collapse: function(p) {
                    ...
                    common.WebEnv.Save('rtm_web_leftMenuCollapse', true);
                ...
                beforeexpand: function() {
                    ...
                    common.WebEnv.Save('rtm_web_leftMenuCollapse', false);
                },
                expand: function() {
                    if (this.leftCon.isExpandLock === true) {
                        return false;
    ...
    createGroupList: function() {
        ...
        var isBizGroup = (Comm.webBizGroups && Comm.webBizGroups.length > 0);
    ...
    setServerColors: function() {
        var ix, ixLen;
        var viewIndex   = Comm.web_env_info['xm-dock-save-rtm.view.rtmWebView'];
        ...
        if (serverColor) {
            ...
            if (customColor && !Array.isArray(customColor)) {
                realtime.serverColorMap.WEB = customColor.WEB;
            }
            customColor = null;
            serverColor = null;
        }
        ...
        if (!realtime.serverColorMap.WEB) {
            realtime.serverColorMap.WEB = {};
        }

        for(ix = 0, ixLen = Comm.webIdArr.length; ix < ixLen; ix++ ) {
            serverId   = Comm.webIdArr[ix];
            color      = realtime.serverColorMap.WEB[serverId];
            ...
            realtime.serverColorMap.WEB[serverId] = color;

            if (!Comm.webServersInfo[Comm.webIdArr[ix]].labelColor){
                Comm.webServersInfo[Comm.webIdArr[ix]].labelColor = realtime.Colors[ix];
            }
        }
    },
    ...
    setMenuGroupList: function() {
        ...
        var groupName;
        var serverCount = Comm.webIdArr.length;
        var serverID;
        ...
        for (jx = 0; jx < serverCount; jx++) {
            ...
            serverList[serverList.length] = {
                ...
                serverType   : 'WEB',
                labelColor   : realtime.serverColorMap.WEB[serverID]
            };
        }

        // Host List
        for (ix = 0, ixLen = realtime.WebHostList.length; ix < ixLen; ix++) {
            groupName = realtime.WebHostList[ix];
            ...
            for (jx = 0; jx < serverCount; jx++) {
                ...
                if (groupName === Comm.RTComm.getWebGroupNameByType(0, serverID)) {
                    serverList[serverList.length] = {
                        isTitle      : false,
                        groupType    : 0,
                        groupName    : groupName,
        ...
        // Business List
        for (ix = 0, ixLen = Comm.webBizGroups.length; ix < ixLen; ix++) {
            groupName = Comm.webBizGroups[ix];
            ...
            for (jx = 0; jx < serverCount; jx++) {
                ...
                if (groupName === Comm.RTComm.getWebGroupNameByType(1, serverID)) {
                    serverList[serverList.length] = {
                        isTitle      : false,
                        groupType    : 1,
                        groupName    : groupName,
        ...
        this.menuBarGroup.frameChange = function() {
            rtmBase.frameChange(realtime.selectedWebNames);
        };
        serverList = null;
        ...
        if (windowFloatingList.length) {
            for (ix = 0, ixLen = windowFloatingList.length; ix < ixLen; ix++) {
                ...
                if (windowComponent &&
                    ...
                    windowComponent.monitorType === 'WEB' &&
    ...
    wasSelect: function(serverName) {
        ...
        if (Ext.isEmpty(serverName)) {
            Comm.selectedWebArr.length = 0;
        }

        var serverId  = Comm.RTComm.getWebIdByName(serverName);
        var isContain = Ext.Array.contains(Comm.selectedWebArr, serverId);

        if (Comm.selectedWebArr.length === Comm.webIdArr.length) {
            Comm.selectedWebArr.length = 0;
            realtime.selectedWebNames.length = 0;

            Comm.selectedWebArr.push(serverId);
            realtime.selectedWebNames.push(serverName);

        } else if (isContain || Ext.isEmpty(serverName)) {
            Ext.Array.remove(Comm.selectedWebArr, serverId);
            Ext.Array.remove(realtime.selectedWebNames, serverName);

            if (Comm.selectedWebArr.length === 0) {
                Comm.selectedWebArr = Comm.webIdArr.concat();
            }
        ...
        this.frameChange(realtime.selectedWebNames);
    },
    ...
    frameRefresh: function() {
        ...
        if (Comm.rtmWebShow === true) {
            var serverId;
            var data;

            for (var ix = 0, ixLen = Comm.webIdArr.length; ix < ixLen; ix++) {
                serverId = Comm.webIdArr[ix];

                data = Repository.OsStatExtend[serverId];
    ...
    parseData: function(serverId, data) {
        ...
        if (Comm.webServersInfo[serverId]) {
            hostName = Comm.webServersInfo[serverId].host;
        }
        ...
        if (index === -1) {
            ...
            wasStatData.cpu      = +data.OS_CPU;
            wasStatData.mem      = +data.OS_MEM;
        }
    ...
    startFrameDraw: function() {
        ...
        if (!this.dockLayer || !Comm.rtmWebShow) {
            return;
        }
        ...
        if (windowFloatingList.length) {
            for (ix = 0, ixLen = windowFloatingList.length; ix < ixLen; ix++) {
                ...
                if (windowComponent && windowComponent.frameRefresh && windowComponent.openViewType === 'WEB') {

                    if (windowComponent.title === common.Util.TR('Activity Monitor') && Comm.rtmWebShow) {
        ...
        // WEB 모니터링 화면에 표시되고 있는 각 컴포넌트를 새로고침한다.
        for(ix = 0, ixLen = this.dockLayer.dockList.length; ix < ixLen; ix++){
    ...
    resetPAWebInfo: function() {
        ...
        if ( !Comm.web_env_info['pa_web_performance_trend_stat_Default']) {
            ...
            common.WebEnv.insert_config( 'pa_web_performance_trend_types',     'Default' ) ;
            common.WebEnv.insert_config( 'pa_web_performance_trend_last_type', 'Default' ) ;

            common.WebEnv.insert_config( 'pa_web_performance_trend_stat', default_stat, 'Default' ) ;
            common.WebEnv.insert_config( 'pa_web_performance_trend_stat_id', default_stat_id, 'Default' ) ;
        ...
        if (!Comm.web_env_info['pa_web_performance_trend_all_stat_Default']) {
            ...
            common.WebEnv.insert_config( 'pa_web_performance_trend_all_types',     'Default' ) ;
            common.WebEnv.insert_config( 'pa_web_performance_trend_all_last_type', 'Default' ) ;

            common.WebEnv.insert_config( 'pa_web_performance_trend_all_stat', default_stat, 'Default' ) ;
            common.WebEnv.insert_config( 'pa_web_performance_trend_all_stat_id', default_stat_id, 'Default' ) ;

            isExecuteReconfig = true;
        }

        if ( !Comm.web_env_info['pa_web_comparison_stat_Default']) {
            ...
            common.WebEnv.insert_config( 'pa_web_comparison_types',     'Default' ) ;
            common.WebEnv.insert_config( 'pa_web_comparison_last_type', 'Default' ) ;

            common.WebEnv.insert_config( 'pa_web_comparison_stat'   , default_comparison_stat  , 'Default' ) ;
            common.WebEnv.insert_config( 'pa_web_comparison_stat_id', default_comparison_stat_id, 'Default' ) ;
