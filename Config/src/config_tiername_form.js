Ext.define('config.config_tiername_form', {
    ...
    init: function (state) {
        ...
        this.was_grid = Ext.create('Exem.adminTree', {
            width : '100%',
            height: '100%',
            editMode: false,
            useCheckBox: false,
        ...
        this.was_grid.addColumn({text: common.Util.CTR('Name') ,         dataIndex: 'tier_name',    width: 80, type: Grid.tree      , alowEdit: false, editMode: false});
        this.was_grid.addColumn({text: common.Util.CTR('Tier Type'),     dataIndex: 'tier_type',    width: 80, type: Grid.StringNumber, alowEdit: false, editMode: false});
        ...
        this.batchCheckBox = Ext.create('Ext.form.field.Text', {
            x: 0,
            y: 64,
            width: 270,
            labelWidth: 80,
        ...
        panelA2.add(this.wasNameEdit, this.hostNameEdit, this.batchCheckBox);
        panelA.add(panelA2);
    ...
    was_load: function() {
        ...
        WS.SQLExec(dataSet, function(aheader, adata) {
            if (adata.rows != undefined) {
                self.was_grid.clearNodes();

                var tier = self.was_grid.addNode(null, ['Tier Name1', 'TYPE', 'DESC']);
                self.was_grid.addNode(tier, ['Tier Name2', 'TYPE', 'DESC']);
                self.was_grid.addNode(tier, ['Tier Name3', 'TYPE', 'DESC']);

                self.was_grid.drawTree();
