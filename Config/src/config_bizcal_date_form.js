Ext.define('config.config_bizcal_date_form', {
    ...
    init: function (state) {
        ...
        if (state == 'Add') {
            form.setTitle(common.Util.TR('Add Date'));
        } else {
        ...
        this.comboData.push ({ name: '급여일', value: '급여일' });
        this.comboData.push ({ name: '집중거래일', value: '집중거래일' });

        this.hostNameCombo.setData(this.comboData);
        this.hostNameCombo.setSearchField('name');

        panelA2.add(this.wasNameEdit, this.hostNameCombo);
        panelA.add(panelA2);
    ...
    was_load: function() {
        ...
        WS.SQLExec(dataSet, function(aheader, adata) {
            if (adata.rows != undefined) {
                ...
                self.wasNameEdit.setValue('2019-04-10');
    ...
    save: function() {
        ...
        var hostname = self.hostNameCombo.getValue();
