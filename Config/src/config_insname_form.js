Ext.define('config.config_insname_form', {
    ...
    init: function (state) {
        ...
        this.was_grid.addColumn({text: common.Util.CTR('Instance ID'),  dataIndex: 'was_id',    width: 90,  type: Grid.StringNumber, alowEdit: false, editMode: false});
        this.was_grid.addColumn({text: common.Util.CTR('Type'),         dataIndex: 'was_name',  width: 100, type: Grid.String, alowEdit: false, editMode: false});
        this.was_grid.addColumn({text: common.Util.CTR('Name'),         dataIndex: 'host_name', width: 100, type: Grid.String, alowEdit: false, editMode: false});
        ...
        this.batchCheckBox = Ext.create('Ext.form.field.Checkbox', {
            ...
            labelWidth: 80,
            labelAlign: 'right',
            fieldLabel: Comm.RTComm.setFont(9, common.Util.CTR('사용여부')),
            allowBlank: true
        });

        panelA2.add(this.wasIdEdit, this.wasNameEdit, this.nameEdit);
        if(this.mode == 'Edit'){
    ...
    was_load: function() {
        ...
        WS.SQLExec(dataSet, function(aheader, adata) {
            if (adata.rows != undefined) {
                ...
                self.was_grid.addRow(['DB1', 'DB', 'Oracle']);
                self.was_grid.addRow(['DB1', 'DB', 'Oracle']);
                self.was_grid.addRow(['DB1', 'DB', 'Oracle']);
                ...
                switch (this.mode) {
                    ...
                    case 'Edit' :
                        ...
                        self.nameEdit.setValue(self.hostname);
