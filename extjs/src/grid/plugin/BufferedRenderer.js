...
Ext.define('Ext.grid.plugin.BufferedRenderer', {
    ...
    loadId: 0,
	/*
	view.body.dom.clientHeight  
	*/
	viewBodyClientHeight : 0,
	viewBodyClientoffsetHeight : 0,
...
*/
    ...
    refreshSize: function() {
        ...
        if (nodeCache.count && nodeCache.endIndex === (me.store.getCount()) - 1) {
            newScrollHeight = me.scrollHeight = me.bodyTop + me.viewBodyClientoffsetHeight;
        }
    ...
    onViewResize: function(view, width, height, oldWidth, oldHeight) {
        ...
        if (!oldHeight || height !== oldHeight) {
			...
			me.viewBodyClientHeight =  view.body.dom.clientHeight;
			me.viewBodyClientoffsetHeight = view.body.dom.offsetHeight;
        }
    ...
    onWrappedColumnWidthChange: function(oldWidth, newWidth) {
        ...
        if (me.store.getCount() && me.bodyTop) {
            ...
            else if (newWidth > oldWidth && me.bodyTop + me.viewBodyClientoffsetHeight - 1 > me.scrollHeight) {
                me.setBodyTop(Math.max(0, me.scrollHeight - me.viewBodyClientoffsetHeight));
            }

            // The bodyTop calculated by renderRange left the body outside the viewport; center it on the viewport.
            else if (me.bodyTop > me.scrollTop || me.bodyTop + me.viewBodyClientoffsetHeight < me.scrollTop + view.getHeight(true)) {
    ...
    stretchView: function(view, scrollRange) {
        ...
        if (me.scrollTop > scrollRange) {
            me.position = me.scrollTop = scrollRange - me.viewBodyClientoffsetHeight;
    ...
    scrollTo: function(recordIdx, options) {
        ...
        store.getRange(startIdx, endIdx, {
            callback: function(range, start, end) {
                ...
                if (direction === 1) {
                    scrollDecrement = me.viewClientHeight - targetRow.offsetHeight;
                }
                me.position = me.scrollTop = Math.min(Math.max(0, tableTop - view.body.getOffsetsTo(targetRow)[1]) - scrollDecrement, scroller.getSize().y - me.viewClientHeight);
                if (lockingPartner) {
    ...
    bottomOfViewCloseToEdge: function() {
        ...
        if (me.variableRowHeight) {
            return me.bodyTop + me.me.viewBodyClientoffsetHeight < me.scrollTop + me.view.lastBox.height + (me.numFromEdge * me.rowHeight);
        } else {
    ...
    getFirstVisibleRowIndex: function(startRow, endRow, viewportTop, viewportBottom) {
        ...
        if (rows.getCount() && me.variableRowHeight) {
            if (!arguments.length) {
                ...
                if (bodyTop > viewportBottom || bodyTop + me.viewBodyClientoffsetHeight < viewportTop) {
                    me.teleported = true;
    ...
    getScrollHeight: function(calculatedOnly) {
        ...
        if (!me.hasOwnProperty('rowHeight')) {
			//console.time('rows.getCount()')
            rowCount = rows.getCount();
			//console.timeEnd('rows.getCount()')

			//console.time('rows.first(true).offsetHeight')
				//rows.first(true).offsetHeight
			//me.rowHeight
			//console.timeEnd('rows.first(true).offsetHeight')

            if (rowCount) {
                //me.rowHeight = me.variableRowHeight ? Math.floor(view.body.dom.clientHeight / rowCount) : rows.first(true).offsetHeight;
                me.rowHeight = me.variableRowHeight ? Math.floor(me.viewBodyClientHeight / rowCount) : me.rowHeight;
            }
        }
        ...
        if (!calculatedOnly) {
            ...
            if (scrollHeight && (rows.endIndex === recCount - 1)) {
                //scrollHeight = Math.max(scrollHeight, me.bodyTop + me.viewBodyClientoffsetHeight - 1);
				scrollHeight = Math.max(scrollHeight, me.bodyTop + me.viewBodyClientoffsetHeight - 1);
            }
        }
