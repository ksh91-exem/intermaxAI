...
//            usePager : false,
        ...
        /**
         //self.summary_bind_Grid.contextMenu.addItem({
        //    title: common.Util.TR('Transaction Summary'),                    // 20분 + 1200000
        //    fn: function() {
        //        var record = this.up().record;
        //        var txnHistory = common.OpenView.open('TxnHistory', {
        //            fromTime     : Ext.util.Format.date(record['TIME'], Comm.dateFormat.HM),
        //            toTime       : Ext.util.Format.date(new Date(+new Date(record['TIME']) + 1200000), Comm.dateFormat.HM),
        //            transactionTF: common.Util.cutOffTxnExtName(record['txn_name'])
        //        });
        //        setTimeout(function (){
        //            txnHistory.retrieve();
    ...
    selectRows: function(adata) {
        ...
        if (self.Tabpnl.getActiveTab().itemId == 'txn_Tab'){
            ...
            var txn_second_dataset = {};
            var txn_bind_dataset = {};
        ...
        if (adata.rows.length > 0) {
            switch(command){
                ...
                case self.sql.activeTxn:
                    ...
                    for (ix = 0, ixLen = adata.rows.length; ix < ixLen; ix++) {
                        dataRows = adata.rows[ix];
                        var tmp = common.Util.convertBindList(dataRows[41]);
                        var resultTmp = [];
                        var loginName = dataRows[38].split(' ')[0];
