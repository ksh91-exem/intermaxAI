 ...
 */
Ext.define("view.ReportAgentMonthly", {
    ...
    innerInit: function () {
        ...
        this.setRadioValue('Last Month', true);
        this._dateClick('Last Month');

        this.monitorTypeSQL = this.sql[this.monitorType];
    },

    executeAgentMonthly: function(execInfo){
        var ix, ixLen;
        var was_ids;
        ...
        if(execInfo.isTimeWindowChecked){
            switch(Comm.currentRepositoryInfo.database_type) {
                ...
                case 'Oracle' :
                    ...
                    break;
                case 'MSSQL' :
                    execParams.timeWindow = 'and substring(convert(nvarchar(19), :time_col, 120), 12,5) >=\'' + execInfo.from_time_window + '\' and substring(convert(nvarchar(19), :time_col, 120), 12,5) <= \'' + execInfo.to_time_window + '\'';
                    break;
                default :
            ...
            switch(Comm.currentRepositoryInfo.database_type) {
                case 'PostgreSQL' :
                    execParams.excludeWeekend =
                        'and to_char(:time_col, \'yyyy-mm-dd\') in (select to_char(the_day, \'yyyy-mm-dd\') from generate_series(timestamp ' + execParams.from_time + ', timestamp ' + execParams.to_time + ', interval \'1 day\') the_day where extract(\'ISODOW\' from the_day) < 6)';
                    break;
                case 'Oracle' :
                    execParams.excludeWeekend =
                        'and to_char(:time_col, \'yyyy-mm-dd\') in (select dt from (select to_char(sdt + level - 1, \'yyyy-mm-dd\') dt, to_char(sdt+ level -1 , \'d\') d from (select to_date(' + execParams.from_time +', \'yyyy-mm-dd hh24:mi:ss\') sdt, to_date(' + execParams.to_time + ', \'yyyy-mm-dd hh24:mi:ss\') edt from dual) connect by level <= edt - sdt + 1) where d not in (\'1\', \'7\'))';
                    break;
                case 'MSSQL' :
                    execParams.excludeWeekend =
        ...
        this.executeAgentMonthlyData('overall', execParams);
        this.executeAgentMonthlyData('performance_summary', execParams);
        this.executeAgentMonthlyData('agent_summary', execParams);
        this.executeAgentMonthlyData('agent_detail', execParams);
        this.executeAgentMonthlyData('alert_detail', execParams);
    },

    executeAgentMonthlyData: function(type, execParams){
        var dataSet = {};
        ...
        WS.SQLExec(dataSet, this.onAgentMonthlyData, this);
        this.executeCount++;
    },


    onAgentMonthlyData: function(header, data) {
        var percent;
    ...
    generateAgentMonthlyReport: function(){
        ...
        for(ix=0, ixLen=this.performanceData.length; ix<ixLen; ix++) {
            ...
            monthlyPTData = this.setPTDefaultData(this.performanceData[ix]);

            cellXYPos = this.setPTDataToDataSheet(monthlyPTData, excelObj, cellXYPos[0], cellXYPos[1], this.sheetCnt++, this.performanceData[ix].statName, chartType, this.performanceData[ix].dataType);

            if (!isSetIntLegend && this.useIntLegend && this.performanceData[ix].dataType === 'agent' && monthlyPTData[0][1]) {
                intLegendInfo.data = monthlyPTData.slice(0);
                isSetIntLegend = true;
            }
    ...
    setReportSheetHeader: function(sheet, cellPos) {
        ...
        sheet.addData(currentCellPos, xPos, {value : common.Util.TR('Agent Monthly Report'), metadata: { style:this.styleFormat.subTitle.id }});
    ...
    setPTDataToDataSheet: function(monthlyPTData, excelObj, xPos, yPos, sheetCnt, statName, chartType, dataType){
        ...
        for(ix=0, ixLen=monthlyPTData.length; ix<ixLen; ix++) {
            for(jx=0, jxLen=monthlyPTData[ix].length; jx<jxLen; jx++) {
                excelObj.dataSheets[sheetCnt].addData(ix, jx, monthlyPTData[ix][jx]);
            }
        }

        if(monthlyPTData[0][1] == "") {
            showLegend = false;
        }
        ...
        excelObj.chartFrames[sheetCnt].setChartData(monthlyPTData, monthlyPTData.length-1);
