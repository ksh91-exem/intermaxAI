Ext.define("view.WebTxnHistory", {
    ...
    sql: {
        summary   : 'IMXPA_WebTxnHistory_summary.sql',
        gridClick : 'IMXPA_WebTxnHistory_gridClick.sql',
        txnElapse : 'IMXPA_WebTxnHistory_txnElapse.sql',
        txnDrag   : 'IMXPA_WebTxnHistory_txnDrag.sql'
    },
    listeners: {
        ...
        self.elapseScatterRdoGroup = Ext.create('Exem.FieldContainer', {
            ...
            items : [{
                ...
                listeners : {
                    change: function (field) {
                        if (field.getValue()){
                            self.radioType = 'and http_status >= 400';
                            self.trendChartDbClick( 'click' );
                        }
                    }
                }
    ...
    initSummaryGrid : function(){
        ...
        self.summaryGrid = Ext.create('Exem.BaseGrid', {
            ...
            gridName  : 'pa_web_txnhistory_summary_grid',
        ...
        self.summaryGrid.addColumn(common.Util.CTR('WebServer'),                   'ws_name',        120,  Grid.String,       true,  false);
        self.summaryGrid.addColumn('WS ID',                                        'ws_id',          100, Grid.StringNumber, false, true);
        ...
        self.summaryGrid.addColumn(common.Util.CTR('Elapse Time (MAX)'),           'max_txn_elapse', 110, Grid.Float,        true,  false);
        self.summaryGrid.addColumn(common.Util.CTR('Elapse Time (AVG)'),           'avg_txn_elapse', 110, Grid.Float,        true,  false);
        self.summaryGrid.addColumn(common.Util.CTR('Status 1XX'),                  'status_1',       130, Grid.Number,       true,  false);
        self.summaryGrid.addColumn(common.Util.CTR('Status 2XX'),                  'status_2',       130, Grid.Number,       true,  false);
        self.summaryGrid.addColumn(common.Util.CTR('Status 3XX'),                  'status_3',       130, Grid.Number,       true,  false);
        self.summaryGrid.addColumn(common.Util.CTR('Status 4XX'),                  'status_4',       130, Grid.Number,       true,  false);
        self.summaryGrid.addColumn(common.Util.CTR('Status 5XX'),                  'status_5',       130, Grid.Number,       true,  false);
        ...
        self.summaryGrid.contextMenu.addItem({
            ...
            fn: function() {
                ...
                var txnHistory = common.OpenView.open('WebTxnHistory', {
                    monitorType  : 'WEB',
                    fromTime     : fromTime,
                    toTime       : toTime,
        ...
        self.txnExecuteChart = Ext.create('Exem.chart.CanvasChartLayer', {
            ...
            plotdblclick: function(event, pos, item, xAxis) {
                ...
                if (self.retrieveFlag) {
                    ...
                    xAxisY = xAxis.y;
                    if ( xAxisY.length == 0 || (xAxisY[0] && xAxisY[0]['y'] == null && xAxisY[1] && xAxisY[1]['y'] == null) ) {
                        return;
                    }
    ...
    dragScatterChart: function(pos) {
        ...
        self.detailListGrid = Ext.create('Exem.BaseGrid', {
            ...
            gridName  : 'pa_web_txnhistory_detail_grid'
        });
        ...
        self.detailListGrid.addColumn(common.Util.CTR('WebServer'),        'ws_name',        250, Grid.String,   true,  false);
        self.detailListGrid.addColumn(common.Util.CTR('Status/Type'),      'status_type',    200, Grid.String,   true , false);
        self.detailListGrid.addColumn(common.Util.CTR('Elapsed Time'),     'elapse_time',    110, Grid.Float,    true,  false);
        self.detailListGrid.addColumn(common.Util.CTR('Client IP'),        'client_ip',      120, Grid.String,   true,  false);
        self.detailListGrid.addColumn('WS ID',                             'ws_id',          135, Grid.Number,   false, true);
        ...
        self.detailListGrid.addColumn('STATUS',                            'status',         155, Grid.String,   false, true);
        ...
        self.detailListGrid.addRenderer('status_type', this.exceptionRenderer.bind(this) , RendererType.bar) ;
            ...
            switch(command){
                case self.sql.summary:
                    ...
                    for (ix = 0, ixLen = adata.rows.length; ix < ixLen; ix++) {
                        ...
                        self.summaryGrid.addRow([
                            ...
                            ,dataRows[6]               // txn_elapse_max
                            ,dataRows[7]               // txn_elapse_avg
                            ,dataRows[8]               // status_1
                            ,dataRows[9]              // status_2
                            ,dataRows[10]              // status_3
                            ,dataRows[11]              // status_4
                            ,dataRows[12]              // status_5
                ...
                case self.sql.txnDrag:
                    for (ix = 0, ixLen = adata.rows.length; ix < ixLen; ix++) {
                        ...
                        statusType = dataRows[2] + ' / ' + dataRows[3];

                        self.detailListGrid.addRow([
                             dataRows[0]                              // time
                            ,dataRows[1]                              // ws_name
                            ,statusType                               // status_type
                            ,dataRows[4]                              // elapse_time
                            ,common.Util.hexIpToDecStr(dataRows[5])   // client_ip
                            ,dataRows[6]                              // ws_id
                            ,dataRows[7]                              // tid
                            ,dataRows[8]                              // txn_id
                            ,dataRows[2]                              // status
                            ,dataRows[9]                              // start_time
                        ]);
                    }
                ...
                case self.sql.txnElapse:
                    ...
                    for (ix = 0, ixLen = dataRows.length; ix < ixLen; ix++) {
                        ...
                        if (dataRows[ix][4] == 0) {  /* Exception */
                            self.txnElapseChart.addValue(0, [xaxis, dataRows[ix][2]]);
                        }
                        else {
                            self.txnElapseChart.addValue(1, [xaxis, dataRows[ix][2]]);
                        }
                    }
