Ext.define('view.TuxTxnHistory', {
    ...
    sql: {
        summary   : 'IMXPA_TuxTxnHistory_summary.sql',
        gridClick : 'IMXPA_TuxTxnHistory_gridClick.sql',
        txnElapse : 'IMXPA_TuxTxnHistory_txnElapse.sql',
        sqlElapse : 'IMXPA_TuxTxnHistory_sqlElapse.sql',
        txnDrag   : 'IMXPA_TuxTxnHistory_txnDrag.sql',
        sqlDrag   : 'IMXPA_TuxTxnHistory_sqlDrag.sql'
    },
    listeners: {
        ...
        self.thirdGrid = Ext.create('Exem.BaseGrid', {
            ...
            itemdblclick: function(dv, record) {
                if (record.data.txn_elapse !== undefined) {
                    ...
                    winSQLFullText = Ext.create('Exem.FullSQLTextWindow');
    ...
    trendChartDbClick: function(event_idx) {  /* sqlElapse 차트 갱신 주기 체크 용도. */
        ...
        var fromTime = common.Util.getDate(dataTime),
            toTime,
            txnElapseDataset = {}, sqlElapseDataset = {};
        ...
        if (adata.rows.length > 0) {
            switch (command) {
                case self.sql.summary:
                    grid = self.summaryGrid.pnlExGrid;
