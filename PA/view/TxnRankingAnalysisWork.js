Ext.define('view.TxnRankingAnalysisWork', {
    extend: 'Exem.Container',
    width: '100%',
    height: '100%',
    layout : 'vbox',
    sql : {
        topRankPanel : 'IMXPA_TxnRankingAnalysis_TopRankPanel.sql',
        botDetailPanel : 'IMXPA_TxnRankingAnalysis_BotDetailPanel.sql'
    },
    ...
    clearAll: function() {
        ...
        this.selectedTxnName = null;
        this.selectedSectionIdx = 0;
        this.rawData = null;

        if(this.txnIdField) {
            this.txnIdField.setValue('');
        }

        if(this.rankTxnDetailGrid) {
            this.rankTxnDetailGrid.clearRows();
        }

        if(this.txnStackChart) {
            this.txnStackChart.clearValues();
            this.txnStackChart.plotDraw();
        }
    ...
    initLayout: function() {
                ...
                tabchange: function( tabPanel, newCard ) {
                    ...
                    if ( !this.topTabFlag[tab] ) {
                        ...
                        if ( baseGrid && baseGrid.pnlExGrid.getStore().getAt(0) ) {
                            this.selectedTxnId = baseGrid.pnlExGrid.getStore().getAt(0).data.txnId;
                            this.selectedTxnName = baseGrid.pnlExGrid.getStore().getAt(0).data.txnName;
                        }
        ...
        this.txnGridStackChartCon = Ext.create('Ext.container.Container', {
            width  : '100%',
            minWidth : 700,
            height: 540,
            flex : 0.2,
        ...
        this.txnLineChartCon = Ext.create('Ext.container.Container', {
            width  : '100%',
            height: 540,
            flex : 0.8,
            minHeight: 540,
        ...
        for ( ix = 0, ixLen = gridKeyList.length; ix < ixLen; ix++ ) {
            this.rankTabPanel.add(this.createCon( gridKeyList[ix], null, null, '10px 10px 10px 10px' ));
        }

        bottomCon.add(this.txnGridStackChartCon, this.txnLineChartCon);
    ...
    createBottomDetailLayout: function() {
        ...
        /* Bottom Left Layout */
        this.txnIdField = Ext.create('Exem.TextField', {
        ...
        this.rankTxnDetailGrid = Ext.create('Exem.BaseGrid', {
            width  : '100%',
            height : 170,
            margin : '12 0 10 6',
            gridName  : 'txnRankAnalysisGrid',
            flex   : null,
            border : true,
            usePager : false,
            borderVisible : true,
        ...
        this.rankTxnDetailGrid.addCls('ranking-anaylsis-grid');

        this.rankTxnDetailGrid.beginAddColumns();
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Date')                       , 'date'           , 80  , Grid.String   , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Transaction Execution Count'), 'txn_exec_count' , 150 , Grid.Number   , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('SQL Execution Count')        , 'sql_exec_count' , 130 , Grid.Number   , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Elapse Time (MAX)')          , 'max_txn_elapse' , 100 , Grid.Float    , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Elapse Time (AVG)')          , 'avg_txn_elapse' , 100 , Grid.Float    , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('SQL Elapse (MAX)')           , 'max_sql_elapse' , 80  , Grid.Float    , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('SQL Elapse (AVG)')           , 'avg_sql_elapse' , 80  , Grid.Float    , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Prepare Count')              , 'prepare_count'  , 100 , Grid.Number   , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Fetch Count')                , 'fetch_count'    , 100 , Grid.Number   , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Open Conn')                  , 'open_conn'      , 100 , Grid.Number   , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Close Conn')                 , 'close_conn'     , 100 , Grid.Number   , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Open Stmt')                  , 'open_stmt'      , 100 , Grid.Number   , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Close Stmt')                 , 'close_stmt'     , 100 , Grid.Number   , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Open RS')                    , 'open_rs'        , 100 , Grid.Number   , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Close RS')                   , 'close_rs'       , 100 , Grid.Number   , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Exception')                  , 'exception'      , 100 , Grid.Number   , true, false);
        this.rankTxnDetailGrid.addColumn(common.Util.CTR('Fetch Time')                 , 'fetch_time'     , 100 , Grid.Number   , true, false);
        this.rankTxnDetailGrid.endAddColumns();

        this.rankTxnDetailGrid.loadLayout(this.rankTxnDetailGrid.gridName);

        this.rankTxnDetailGrid.addRenderer('close_stmt', this.noClosedRenderer, RendererType.bar);
        this.rankTxnDetailGrid.addRenderer('close_conn', this.noClosedRenderer, RendererType.bar);
        this.rankTxnDetailGrid.addRenderer('close_rs',   this.noClosedRenderer, RendererType.bar);

        this.txnStackChart = Ext.create('Exem.chart.CanvasChartLayer', {
            title        : common.Util.TR('Elapse Time & Execute Count'),
            layout       : 'fit',
            height       : '50%',
            minHeight    : 70,
            width        : '100%',
            itemId       : 'rank-txn-stack-chart',
        ...
        this.txnGridStackChartCon.add(this.txnIdField, this.rankTxnDetailGrid, this.txnStackChart);
        ...
        this.timeFormatToggle = Ext.create('Ext.ux.toggleslide.ToggleSlide', {
            ...
            listeners: {
                change: function() {
                    this.drawTxnDetailLineChart(this.rawData);
                }.bind(this)
            }
        });
        ...
        this.txnExecuteChart = Ext.create('Exem.chart.CanvasChartLayer', {
            layout : 'fit',
        ...
        this.txnMaxElapseChart = Ext.create('Exem.chart.CanvasChartLayer', {
            layout : 'fit',
            title  : common.Util.TR('Transaction Elapse Time (MAX)'),
            split  : true,
            height : '33%',
            interval         : PlotChart.time.exHour,
            minHeight        : 120,
        ...
        this.txnAvgElapseChart = Ext.create('Exem.chart.CanvasChartLayer', {
            layout : 'fit',
            title  : common.Util.TR('Transaction Elapse Time (AVG)'),
            split  : true,
            height : '33%',
            interval         : PlotChart.time.exHour,
            minHeight        : 120,
        ...
        this.lineChartArr = [this.txnExecuteChart, this.txnMaxElapseChart, this.txnAvgElapseChart];
        this.txnLineChartCon.add(this.timeFormatContainer, this.txnExecuteChart, this.txnMaxElapseChart, this.txnAvgElapseChart);

    },

    executeSQL: function(param) {
        // parameter는 실제 데이터를 추출하기 위한 기본 작업
        // onSQLView로 데이터 추출 쿼리 프레임을 가져옴
        if(!this.loadCompleted) {
            return;
        }
        ...
        this.elapseBase = param.elapseBase;

        //extend condition
        this.minElapse = param.minElapse;
        this.txnName = param.txnName;
    ...
    execTopPanelGrid: function(sqlTxt) {
        ...
        // extend condition
        var txnNameSQL = this.txnName.length ? this.txnName : '%',
            minElapseSQL = '';


        var topTabSqlObj = { 'Elapse Time' : {}, 'Execute Count' : {} };

        // Dynamic Query Setting.
        // Elapse Time Parts.
        topTabSqlObj['Elapse Time'] = {
            oriValueStr: [
                'round(cast(sum( cast( txn_elapse_sum as float ) ) / cast( case sum( cast( txn_exec_count as float ) ) when 0 then 1 else sum( cast( txn_exec_count as float ) ) end as float ) / 1000.0 as numeric ), 3) txn_elapse_avg',
                'round(cast(max( cast( txn_elapse_max as float ) ) / 1000.0 as numeric ), 3) txn_elapse_max'
            ],
            valuePerStr: [
                'round( x.txn_elapse_avg / case t.total_txn_elapse_avg when 0 then 1 else t.total_txn_elapse_avg end * 100, 1 ) elapse_avg_per',
                'round( x.txn_elapse_max / case t.total_txn_elapse_max when 0 then 1 else t.total_txn_elapse_max end * 100, 1 ) elapse_max_per'
            ],
            valueStr    : ['x.txn_elapse_avg', 'x.txn_elapse_max'],
            value       : ['txn_elapse_avg', 'txn_elapse_max'],
            orderByCol  : ['txn_elapse_avg', 'txn_elapse_max'],
            totValueStr : ['sum(txn_elapse_avg) total_txn_elapse_avg', 'sum(txn_elapse_max) total_txn_elapse_max'],
            columnPerStr: ['elapse_avg_per', 'elapse_max_per']
        };

        //////////////////////////
        // Execute Count Parts. //
        //////////////////////////
        topTabSqlObj['Execute Count'] = {
            oriValueStr: [
                'sum(cast(txn_exec_count as float) ) txn_exec_count',
                'round(cast(sum( cast( txn_elapse_sum as float ) ) / cast( case sum( cast( txn_exec_count as float ) ) when 0 then 1 else sum( cast( txn_exec_count as float ) ) end as float ) / 1000.0 as numeric ), 3) txn_elapse_avg',
                'round(cast(max( cast( txn_elapse_max as float ) ) / 1000.0 as numeric ), 3) txn_elapse_max'
            ],
            valuePerStr : ['round( cast(x.txn_exec_count as numeric) / cast(case t.total_txn_exec_count when 0 then 1 else t.total_txn_exec_count end as numeric) * 100, 2 ) exec_count_per'],
            valueStr    : ['x.txn_exec_count'],
            value       : ['txn_exec_count'],
            orderByCol  : ['txn_exec_count'],
            totValueStr : ['sum(txn_exec_count) total_txn_exec_count'],
            columnPerStr: ['exec_count_per']
        }


        // Repo에 따른 Data 정합성을 위한 쿼리 변경 처리
        if(Comm.currentRepositoryInfo.database_type == 'MSSQL') {
            topTabSqlObj['Elapse Time'].oriValueStr = [
                'round( sum( convert(float , txn_elapse_sum ) ) / convert( float , case sum( convert( float , txn_exec_count ) ) when 0 then 1 else sum( convert( float , txn_exec_count ) ) end ) /convert( float , 1000 ) , 3 ) txn_elapse_avg',
                'round( max( convert(float , txn_elapse_max ) ) / convert( float, 1000 ) , 3 ) txn_elapse_max'
            ];
            topTabSqlObj['Execute Count'].oriValueStr = [
                'sum(cast(txn_exec_count as float) ) txn_exec_count',
                'round( sum( convert(float , txn_elapse_sum ) ) / convert( float , case sum( convert( float , txn_exec_count ) ) when 0 then 1 else sum( convert( float , txn_exec_count ) ) end ) /convert( float , 1000 ) , 3 ) txn_elapse_avg',
                'round( max( convert(float , txn_elapse_max ) ) / convert( float, 1000 ) , 3 ) txn_elapse_max'
            ];
        }
        else if(Comm.currentRepositoryInfo.database_type == 'Oracle') {
            topTabSqlObj['Elapse Time'].oriValueStr = [
                'round( sum( txn_elapse_sum ) / case sum( txn_exec_count  ) when 0 then 1 else sum( txn_exec_count ) end  / 1000.0, 3) txn_elapse_avg',
                'round( max(txn_elapse_max) / 1000.0 , 3) txn_elapse_max'
            ];
        ...
        tmpCnt = 0;
        tab = this.rankTabPanel.getActiveTab().itemId;
        ...
        for(ix=0, ixLen=sqlObjKeys.length; ix<ixLen; ix++) {
            for(jx=0, jxLen=topTabSqlObj[tab][sqlObjKeys[ix]].length; jx<jxLen; jx++) {
                ...
                if(sqlObjKeys[ix] == 'valuePerStr') {
                    if(tab == 'Elapse Time') {
                        if(this.elapseBase == 'avg') {
                            sqlOriValuePerStr += topTabSqlObj[tab][sqlObjKeys[ix]][0];
                        }
                        else {
                            sqlOriValuePerStr += topTabSqlObj[tab][sqlObjKeys[ix]][1];
                        }
                    }
                    else {
                        sqlOriValuePerStr = topTabSqlObj[tab][sqlObjKeys[ix]][0];
                ...
                if(sqlObjKeys[ix] == 'orderByCol') {
                    if(tab == 'Elapse Time') {
                        if(this.elapseBase == 'avg') {
                            ...
                            sqlOrderByCol += topTabSqlObj[tab][sqlObjKeys[ix]][1];
                        }
                    }
                    else {
                        sqlOrderByCol = topTabSqlObj[tab][sqlObjKeys[ix]][0];
                ...
                if(sqlObjKeys[ix] == 'totValueStr') {
                    if(tab == 'Elapse Time') {
                        if(this.elapseBase == 'avg') {
                            ...
                            sqlTotValueStr += topTabSqlObj[tab][sqlObjKeys[ix]][1];
                        }
                    }
                    else {
                        sqlTotValueStr = topTabSqlObj[tab][sqlObjKeys[ix]][0];
                ...
                if(sqlObjKeys[ix] == 'columnPerStr') {
                    if(tab == 'Elapse Time') {
                        if(this.elapseBase == 'avg') {
                            ...
                            sqlValuePerStr += topTabSqlObj[tab][sqlObjKeys[ix]][1];
                        }
                    }
                    else {
                        sqlValuePerStr = topTabSqlObj[tab][sqlObjKeys[ix]][0];
        ...
        for ( ix = 0, ixLen = this.gridDateList.length; ix < ixLen; ix++ ) {
            ...
            createFirstTableSQL = createFirstTableSQL.replace(/:txnName/gi, '\'' + txnNameSQL + '\'');
            ...
            // extend condition
            if(this.minElapse > 0) {
                minElapseSQL = (this.elapseBase == 'avg' ? 'and txn_elapse_avg >= ' : 'and txn_elapse_max >= ') + this.minElapse;
                createFirstTableSQL = createFirstTableSQL.replace(/:minElapse/gi, minElapseSQL);
            }
            else {
                createFirstTableSQL = createFirstTableSQL.replace(/:minElapse/gi, '');
            }
        ...
        resultSQL =
            '/* Ranking Analysis ( IMXPA_TxnRankingAnalysis_TopPanelGrid.sql ) */\n' +
            createFirstTableSQL +
            createSecondTableSQL +
            '\n-- TxnId별 순위 변동 추이 trend 확인\n' + selectSQL + ';\n' +
            truncateFirstTableSQL +
            truncateSecondTableSQL +
            dropFirstTableSQL +
            dropSecondTableSQL;
    ...
    execBotDetailPanel : function(sqlTxt) {
        ...
        oriTmpSqlStr = oriTmpSqlStr.replace(/:txn_id/gi, '\''+this.selectedTxnId+'\'');
        ...
        resultSQL =
            '/* Ranking Analysis - (IMXPA_TxnRankingAnalysis_BotDetailPanel.sql ) */\n' +
            dayGrpSQL + ';\n' + hourGrpSQL + ';\n' + tenMinGrpSQL + ';\n';
    ...
    drawBotDetailPnl: function(data) {
        this.txnIdField.setValue(this.selectedTxnName);

        this.drawTxnDetailGrid(data[0].rows);
        this.drawTxnDetailStackChart(data[0].rows);
        this.drawTxnDetailLineChart(data);

        this.loadCompleted = true;
        this.loadingMask.hide();
    },

    drawTxnDetailGrid: function(data) {
        var ix, ixLen,
            dataRows;
        ...
        for(ix=0, ixLen=data.length; ix<ixLen; ix++) {
            ...
            this.rankTxnDetailGrid.addRow([
                 Ext.util.Format.date(dataRows[0], 'Y-m-d')     //date
                ,dataRows[1]     //txn_exec_count
                ,dataRows[2]     //sql_exec_count
                ,dataRows[3]     //max_txn_elapse
                ,dataRows[4]     //avg_txn_elapse
                ,dataRows[5]     //max_sql_elapse
                ,dataRows[6]     //avg_sql_elapse
                ,dataRows[7]     //prepare_count
                ,dataRows[8]     //fetch_count
                ,+dataRows[9]     //open_conn
                ,+dataRows[10]    //close_conn
                ,+dataRows[11]    //open_stmt
                ,+dataRows[12]    //close_stmt
                ,+dataRows[13]    //open_rs
                ,+dataRows[14]    //close_rs
                ,dataRows[15]    //exception
                ,dataRows[16]    //fetch_time
            ]);
        }

        this.rankTxnDetailGrid.drawGrid();
    },

    drawTxnDetailStackChart: function(data) {
        ...
        for(ix=0, ixLen=TOPtxnStore.length; ix<ixLen; ix++) {
            var xaxis = Ext.util.Format.date(TOPtxnStore[ix][0], 'm-d');

            this.txnStackChart.addValue(0, [xaxis, TOPtxnStore[ix][2]]);
            this.txnStackChart.addValue(1, [xaxis, TOPtxnStore[ix][1]]);
            this.txnStackChart.addValue(2, [xaxis, TOPtxnStore[ix][3]]);
        }

        this.txnStackChart.plotDraw();
    },

    drawTxnDetailLineChart: function(data) {
    ...
    /* Bottom Txn Detail Grid Sub Function  */
    ...
    /* Bottom Txn Detail DBLclick Sub Function  */
    createBotDetailPopup: function() {
        ...
        grid.addColumn(common.Util.CTR('Elapse Time (MAX)')          , 'max_txn_elapse' , 100 , Grid.Float    , true , false);
        grid.addColumn(common.Util.CTR('Elapse Time (AVG)')          , 'avg_txn_elapse' , 100 , Grid.Float    , true , false);
        grid.addColumn(common.Util.CTR('Transaction Execution Count'), 'txn_exec_count' , 130 , Grid.Number   , true , false);
        grid.endAddColumns();
                ...
                for ( jx = 0, jxLen = data.length; jx < jxLen; jx++ ) {
                    ...
                    if(tabId == 'Elapse Time') {
                        grid.addRow([
                            ...
                            , this.elapseBase == 'avg' ? data[jx][4] : data[jx][5]  // tabValue
                            , this.elapseBase == 'avg' ? data[jx][5] : data[jx][4]  // tabValue
                            , ix            // gridIdx
                        ]);
                    } else {
                        grid.addRow([
                ...
                for ( jx = 0, jxLen = data.length; jx < jxLen; jx++ ) {
                    ...
                    if(tabId == 'Elapse Time') {
                        grid.addRow([
                            ...
                            , this.elapseBase == 'avg' ? data[jx][compareGridRankIdx + 5] : data[jx][compareGridRankIdx + 6]  // tabValue
                            , this.elapseBase == 'avg' ? data[jx][compareGridRankIdx + 6] : data[jx][compareGridRankIdx + 5]  // tabValue
                            , ix                           // gridIdx
                        ]);
                    } else {
                        grid.addRow([
    ...
    columnColorChange: function() {
        ...
        for ( ix = 0, ixLen = conList.length; ix < ixLen; ix++ ) {
            ...
            for ( jx = 0, jxLen = gridList.length; jx < jxLen; jx++ ) {
                ...
                colObj.txnIdCol       = headerCt.down('[dataIndex=txnName]');
    ...
    paintSelectedTxnId: function() {
        var grid, compareCon, findTxnId = false;
        var ix, ixLen, jx, jxLen;
        var rowIdx;
        var conList = this.rankTabPanel.getActiveTab().items.items;
        var gridList;
        ...
        if ( !this.selectedTxnId ) {
            ...
            if ( grid && grid.pnlExGrid.getStore().getCount() && grid.pnlExGrid.getStore().getAt(0) ) {
                this.selectedTxnId = grid.pnlExGrid.getStore().getAt(0).data.txnId;
                this.selectedTxnName = grid.pnlExGrid.getStore().getAt(0).data.txnName;
        ...
        for ( ix = conList.length-1, ixLen = 0; ix >= ixLen; ix-- ) {
            ...
            for ( jx = gridList.length-1, jxLen = 0; jx >= jxLen; jx-- ) {
                rowIdx = gridList[jx].pnlExGrid.getStore().find('txnId', this.selectedTxnId);
    ...
    createTopGrid: function( gridIdx, gridDateList ) {
        ...
        var grid = Ext.create('Exem.BaseGrid', {
            ...
            itemclick : function(dv, record) {
                ...
                this.selectedTxnId      = record.data['txnId'];
                this.selectedTxnName    = record.data['txnName'];
        ...
        grid.addColumn(dateLabel             , 'txnName' , 120 , Grid.String       , true, false);
        grid.addColumn('TXN ID'              , 'txnId'   , 0   , Grid.String       , false, true);
        ...
        arrColValue = tab == 'Elapse Time' ? ['rank', 'txnName', 'value_0', 'value_1'] : ['rank', 'txnName', 'value_0'];
    ...
    metaColorChange: function(meta) {
        if ( this.selectedTxnId && this.selectedTxnId == meta.record.data.txnId ) {
