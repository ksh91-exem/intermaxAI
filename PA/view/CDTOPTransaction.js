Ext.define("view.CDTOPTransaction", {
    ...
    sql : {
        TOPTxnList_GroupAgent : 'IMXPA_CDTOPTransaction_TOPTxnList_GroupAgent.sql',
        TOPTxnList_GroupTxn   : 'IMXPA_CDTOPTransaction_TOPTxnList_GroupTxn.sql',
        ChartQuery : 'IMXPA_CDTOPTransaction_ChartQuery.sql'
    },
    autoScroll: true,
    topTxnList_Obj   : {},
    retrieveFlag : false,
    ...
    init: function(){
        ...
        self.ElapseTime_NumberField = Ext.create('Exem.NumberField', {
            fieldLabel : common.Util.TR('Elapse Time (AVG)' + microSign + ' >='),
        ...
        self.topTxnList_Grid.contextMenu.addItem({
            ...
            fn: function() {
                ...
                if ( active_tab.itemId == 'total' ){
                    txnHistory = common.OpenView.open('CDTxnHistory', {
                        isWindow : false,
                        width    : 1200,
                        height   : 800,
        ...
        self.topTxnList_Grid.addColumn(common.Util.CTR('Transaction')                                , 'txn_name'            , 350, Grid.String, true,  false);
        self.topTxnList_Grid.addColumn(common.Util.CTR('Agent')                                      , 'was_name'            , 140, Grid.String, true,  false);
        self.topTxnList_Grid.addColumn(common.Util.CTR('Transaction Execution Count')                , 'txn_exec_count'      , 140, Grid.Number, true,  false);
        self.topTxnList_Grid.addColumn(common.Util.CTR('Transaction Elapse Time (Total)') + microSign, 'txn_elapse_us'       , 200, Grid.Number, true,  false);
        self.topTxnList_Grid.addColumn(common.Util.CTR('Transaction Elapse Time (MAX)') + microSign  , 'txn_elapse_us_max'   , 200, Grid.Number, true,  false);
        self.topTxnList_Grid.addColumn(common.Util.CTR('Transaction Elapse Time (AVG)') + microSign  , 'txn_elapse_us_avg'   , 200, Grid.Float,  true,  false);
        self.topTxnList_Grid.addColumn(common.Util.CTR('Transaction Elapse Time Standard Deviation') , 'txn_elapse_us_stddev', 110, Grid.Float , true , false);
        ...
        self.ChartDetailGrid.addColumn(common.Util.CTR('Transaction Elapse Time (MAX)') + microSign, 'txn_elapse_us_max',  180, Grid.Number,       true,  false);
        self.ChartDetailGrid.addColumn(common.Util.CTR('Transaction Execution Count'),               'txn_exec_count',     150, Grid.Number,       true,  false);
        self.ChartDetailGrid.addColumn(common.Util.CTR('Transaction Elapse Time (AVG)') + microSign, 'txn_elapse__us_avg', 180, Grid.Float,        true,  false);
    ...
    condition_executeSQL: function(){
        ...
        if (self.retrieveFlag){
            ...
            if (self.orderByRadioContainer.getCheckedValue() == common.Util.TR('txn_elapse_us_avg')){
                self.topTxnList_Grid.setOrderAct('txn_elapse_us_avg', 'DESC');
            }else {
    ...
    executeSQL: function() {
        ...
        if(this.agentRetrieveRadioContainer.getCheckedValue() === 'agent_each'){
            ...
            for(ix = 0, ixLen = wasIds.length; ix < ixLen; ix++) {
                ...
                self.wasName = Comm.wasInfoObj[wasIds[ix]].wasName;
    ...
    GridselectRows: function() {
        ...
        self.wasList = this.wasCombo.getValue();
        wasId = self.TOPtxnList_tabPanel.getActiveTab().itemId;
    ...
    onDrawGridTOPTxnList_Agent: function(adata){
        ...
        for (var ix=0; ix <= adata.rows.length-1; ix++) {
            self.topTxnList_Grid.addRow([
                ...
                ,adata.rows[ix][4]     //txn_elapse_us_sum
                ,adata.rows[ix][5]     //txn_elapse_us_max
                ,adata.rows[ix][6]     //txn_elapse_us_avg
                ,adata.rows[ix][7]     //txn_elapse_us_stddev
            ]);
        }
    ...
    onDrawGridTOPTxnList_Txn: function(adata){
        ...
        for (var ix=0; ix <= adata.rows.length-1; ix++) {
            self.topTxnList_Grid.addRow([
                ...
                ,adata.rows[ix][3]     //txn_elapse_us_sum
                ,adata.rows[ix][4]     //txn_elapse_us_max
                ,adata.rows[ix][5]     //txn_elapse_us_avg
                ,adata.rows[ix][6]     //txn_elapse_us_stddev
            ]);
        }
        self.topTxnList_Grid.drawGrid();
        if(adata.rows.length > 0){
