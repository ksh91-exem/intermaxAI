/**
 * Created by ksh on 2017-12-11.
 */
Ext.define('view.TuxComparisonTrend', {
    ...
    get_chart_sql: function(type, date_obj) {
        var sql_file, diff1, diff2;

        try {
            diff1 = +new Date(date_obj.getFromDateTime() + ':00:00').setMinutes(0);
            diff2 = +new Date(date_obj.getToDateTime() + ':00:00').setMinutes(0);

            if (diff2 - diff1 >= 86400000 ) {
                sql_file = 'IMXPA_TuxComparisonTrend_Chart_Agent_daily.sql';
            } else {
                sql_file = 'IMXPA_TuxComparisonTrend_Chart_Agent.sql';
            }
    ...
    on_chart: function( header, data ) {
        var self = this.self,
            arr_store = {};
        var stat_list, cht_cnt, ix, jx, data_idx, series_id, from_time, to_time, interval, comboCnt;
    ...
    check_valid: function() {
        ...
        var first_obj, second_obj, third_obj;

        var first_was, second_was, third_was;
        var first_time, second_time, third_time;
        ...
        try {
            switch (this.cbo_condition.getValue()) {
                ...
                case '3':
                    ...
                    first_was   = this.was_combo.getValue();
                    second_was  = this.second_was_combo.getValue();
                    third_was   = this.third_was_combo.getValue();

                    first_time  = this.first_date.getFromDateTime();
                    second_time = this.second_date.getFromDateTime();
                    third_time  = this.third_date.getFromDateTime();
    ...
    init: function() {
        ...
        this.IDXDB_STAT      = 'pa_tux_comparison_stat';
        this.IDXDB_STAT_ID   = 'pa_tux_comparison_stat_id';
        this.IDXDB_GC        = 'pa_tux_comparison_gc';
        this.IDXDB_GC_ID     = 'pa_tux_comparison_gc_id';
        this.IDXDB_LAST_TYPE = 'pa_tux_comparison_last_type';
        this.IDXDB_TYPES     = 'pa_tux_comparison_types';
    ...
    layout_center: function() {
        ...
        if ( this.stat_change == undefined || this.stat_change == null ) {
            ...
            setTimeout(function() {
                ...
                this.stat_view.view_name          = 'tux_comparison';
    ...
    title_update: function(arr_cht, title, id) {
        ...
        for (ix = 0, ixLen = arr_cht.length; ix < ixLen; ix++) {
            ...
            arr_cht[ix].stat_id = id[ix];

            arr_cht[ix].removeAllSeries();

            if (id[ix].indexOf(sumLiteral) === -1) {
                seriesLabel = common.Util.CTR('AVG');
            } else {
                seriesLabel = common.Util.CTR('SUM');
            }

            arr_cht[ix].addSeries({
                label: seriesLabel,
                id   : 'mid_cht0',
                type : PlotChart.type.exLine,
                color: '#2b99f0'
