Ext.define('view.TuxResponseInspector', {
    extend: 'Exem.TxnTrendBaseForm',
    title: '',
    sql : {
        elapseTimeScatter     : 'IMXPA_TuxResponseInspector_Scatter.sql',
        detailListGrid        : 'IMXPA_TuxResponseInspector_GridList_paging.sql',
        detailListGrid_Re     : 'IMXPA_TuxResponseInspector_GridList_paging_re.sql',
        detailListGrid_Re_Txn : 'IMXPA_TuxResponseInspector_GridList_paging_re_txn.sql',
        txnChart              : 'IMXPA_ResponseInspector_txn_chart.sql',      // 변경 없음
        serverEnv             : 'IMXPA_TuxResponseInspector_ServerEnv.sql'
    },
    innerInit: function() {
    ...
    createLayoutDetailCond: function() {
        ...
        this.conditionArea.add([this.txnNameField, this.serverNameField, this.tidField, this.gidField]);
    ...
    retrieve: function() {
        var self = this,
            maxElapse, tmpTxnName, txnName, serverName, chkException, gid, tid;
        ...
        if (result) {
            ...
            if (serverName == '%') {
                ...
                this._serverNameRepl = 'AND s.prog_name LIKE \'' + serverName + '\'';
            }
    ...
    addRowDetailGrid: function(data) {
        ...
        for (ix = 0, ixLen = rowData.rows.length; ix < ixLen; ix++) {
            ...
            this.detailListGrid.addRow([
                ...
                ,rowData.rows[ix][ 3]                            // 'server_name'
                ,rowData.rows[ix][ 4]                            // 'cpu'
                ,rowData.rows[ix][ 5]                            // 'start_time'
                ,rowData.rows[ix][ 6]                            // 'txn_elapse'
                ,exceptionType
                ,rowData.rows[ix][ 7]                            // 'sql_elapse'
                ,rowData.rows[ix][ 8]                            // 'host_name'
                ,common.Util.hexIpToDecStr(rowData.rows[ix][9])  // 'client_ip'
                ,rowData.rows[ix][10]                            // 'sql_exec_count'
                ,rowData.rows[ix][11]                            // 'fetch_count'
                ,rowData.rows[ix][12]                            // 'fetch_time'
                ,rowData.rows[ix][13]                            // 'was_id'
                ,rowData.rows[ix][14]                            // 'tid'
                ,rowData.rows[ix][15]                            // 'txn_id'
                ,rowData.rows[ix][16]                            // 'raw_time'
                ,rowData.rows[ix][17]                            // 'exception_count'
                ,rowData.rows[ix][19]                            // 'guid'
                ,rowData.rows[ix][20]                            // 'remote_elapse'
                ,rowData.rows[ix][21]                            // 'remote_count'
    ...
    exceptionTypeProc: function(data) {
        var retExceptionText = '',
            exceptionCnt = data[17],
            exceptionType = data[18];
