...
Ext.application({
    ...
    launch: function() {
        ...
        var groupList = window.opener.APP.datas.instancelist;
        ...
        for(ix = 0, ixLen = groupList.length; ix < ixLen; ix++){
            groupName = groupList[ix].groupname;
            if(groupName){
                Comm.bizGroupWasNamePairObj[groupName] = [];
            }

            instanceList = groupList[ix].server_list;
            for(jx = 0, jxLen = instanceList.length; jx < jxLen; jx++){
                wasId = instanceList[jx].serverid;
                wasName = instanceList[jx].name;
                Comm.wasInfoObj[wasId] = {wasName : instanceList[jx].name};
                Comm.selectedWasArr.push(wasId);
                Comm.wasAppType[wasId] = 'JAVA';
                Comm.wasIdArr.push(wasId);
                Comm.wasNameArr.push(wasName);

                if(groupName){
                    Comm.bizGroupWasNamePairObj[groupList[ix].groupname].push([wasId, wasName]);
                }
            }
        }

        var param = this.getParameter();
        var elapseRange = [param.y1];
        if(param.max != 'Y'){
            elapseRange.push(param.y2);
        }

        var testPanel = Ext.create('view.ResponseInspector',{
            title : common.Util.TR('Transaction Trend'),
            isAllWasRetrieve: true,
            autoRetrieveRange: {
                wasName: 'All',
                timeRange: [
                    new Date(+param.x1), new Date(+param.x2)
                ],
                elapseRange: elapseRange
            },
            reorderable : false
        });

        window.tabPanel.add(testPanel);
    ...
    getParameter: function(){
        var data = null, token = null, result = {}
