Ext.define('Exem.tuxTxnDetail', {
    ...
    addColumnTxnGrid: function() {
        this.txnDetailGrid.gridName = 'rtm_popup_tux_txndetail_grid';
    ...
    executeSQL: function() {
        ...
        if (serverName !== '') {
            serverName = 'AND s.prog_name LIKE \'' + serverName + '\'';
        }
        ...
        if (parameter.msFromTime && parameter.msToTime) {
            dataSet.sql_file = 'IMXRT_tuxTxnDetail_gridList_paging_ms.sql';
            dataSet.bind.push(
            ...
            dataSet.sql_file = 'IMXRT_tuxTxnDetail_gridList_paging.sql';
        }
    ...
    addRowTxnGridData: function(gridData) {
        ...
        for (ix = 0, ixLen = gridData.length; ix < ixLen; ix++) {
            ...
            exceptionType = this.exceptionTypeProc(gridData[ix][18], gridData[ix][19]);

            this.txnDetailGrid.addRow([
                gridData[ix][ 0]                              // 'time'
                ,gridData[ix][ 1]                             // 'was_name'
                ,gridData[ix][ 2]                             // 'txn_name'
                ,gridData[ix][ 3]                             // 'server_name'
                ,gridData[ix][ 4]                             // 'cpu'
                ,gridData[ix][ 5]                             // 'start_time'
                ,gridData[ix][ 6]                             // 'txn_elapse'
                ,exceptionType                                // 'exception_type'
                ,gridData[ix][ 7]                             // 'sql_elapse'
                ,gridData[ix][ 8]                             // 'host_name'
                ,common.Util.hexIpToDecStr(gridData[ix][9])   // 'client_ip'
                ,gridData[ix][10]                             // 'sql_exec_count'
                ,gridData[ix][11]                             // 'fetch_count'
                ,gridData[ix][12]                             // 'fetch_time'
                ,gridData[ix][14]                             // 'was_id'
                ,gridData[ix][15]                             // 'tid'
                ,gridData[ix][16]                             // 'txn_id'
                ,gridData[ix][17]                             // 'raw_time'
                ,gridData[ix][18]                             // 'exception_count'
                ,gridData[ix][20]                             // 'guid'
            ]);
        }
